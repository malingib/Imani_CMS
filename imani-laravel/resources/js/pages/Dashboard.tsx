...
export default function Dashboard({ auth, activeChurch, churches, members, transactions, events }: DashboardPageProps) {
    ...
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-700 pb-10">
      <header className="flex flex-col xl:flex-row xl:items-end justify-between gap-6">
        ...
        <div className="grid grid-cols-2 md:flex gap-3">
          <button onClick={() => router.visit('/communication')} className="flex items-center justify-center gap-2 px-6 py-4 bg-white text-slate-700 border border-slate-200 rounded-[1.25rem] font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm">
            <MessageSquare size={18} className="text-brand-indigo" /> Messages
          </button>
          <button onClick={() => router.visit('/finance')} className="flex items-center justify-center gap-2 px-6 py-4 bg-brand-primary text-white rounded-[1.25rem] font-black text-xs uppercase tracking-widest hover:bg-brand-primary-700 transition-all shadow-xl shadow-brand-primary/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
             {[
               { label: 'Tithe', icon: HandCoins, color: 'emerald', action: () => router.visit('/finance') },
               { label: 'Convert', icon: UserPlus, color: 'indigo', action: () => router.visit('/members') },
               { label: 'Service', icon: CalendarPlus, color: 'primary', action: () => router.visit('/events') },
               { label: 'Outreach', icon: MessageSquare, color: 'gold', action: () => router.visit('/communication') }
             ].map((btn, i) => (
               <button 
                 key={i} 
                 onClick={btn.action}
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-white p-6 rounded-[2rem] shadow-md space-y-6">
              ...
              <button onClick={() => router.visit('/finance')} className="w-full py-4 bg-slate-50 text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-100 transition-all flex items-center justify-center gap-2">
