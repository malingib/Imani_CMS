...
import { Building2, Plus, Search, CheckCircle2, XCircle, AlertCircle, Loader2, RefreshCw } from 'lucide-react';
...
const TenantsList: React.FC<TenantsListProps> = ({ onNavigate, onSelectChurch }) => {
  ...
  const createChurch = async () => {
    setError('');
    if (!newName.trim()) { setError('Church name is required'); return; }

    const slug = newSlug || newName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    if (!slug) { setError('Invalid church name'); return; }

    const { data: existing } = await supabase.from('churches').select('id').eq('slug', slug).maybeSingle();
    if (existing) { setError(`A church with slug "${slug}" already exists`); return; }

    const { data, error: insertError } = await supabase.from('churches').insert([{ name: newName.trim(), slug, tier: newTier }]).select();
    if (insertError) { setError(insertError.message); return; }
    if (!data || data.length === 0) { setError('Failed to create church'); return; }
    ...
    if (newEmail.trim()) {
      ...
      await supabase.from('invitations').insert([{
        church_id: data[0].id, email: newEmail.trim(), role: 'ADMIN', token,
  ...
  const toggleStatus = async (church: Church) => {
    ...
    await supabase.from('churches').update({ status: newStatus }).eq('id', church.id);
    fetch();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetch} className="p-3 text-slate-400 hover:text-brand-primary hover:bg-slate-50 rounded-2xl transition-all">
            <RefreshCw size={18} />
          </button>
              ...
              <div key={church.id} className="flex items-center justify-between p-6 border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
                <div className="flex items-center gap-4 flex-1 cursor-pointer" onClick={() => onSelectChurch(church.id)}>
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-400">{church.memberCount} members · {church.tier} · {church.slug}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <button onClick={() => toggleStatus(church)} className="hover:opacity-80 transition-opacity" title={church.status === 'active' ? 'Suspend' : 'Activate'}>
