...
export default function MembersIndex({
  ...
  const onAddMember = (member: Member) => {
    router.post('/members', { ...member }, { preserveScroll: true, onSuccess: () => addToast('Member saved') });
  };
  const onAddMembersBulk = (imported: Member[]) => {
    router.post('/members/bulk', { members: JSON.parse(JSON.stringify(imported)) }, { preserveScroll: true, onSuccess: () => addToast(`Successfully imported ${imported.length} members`, 'success') });
  };
  const onUpdateMember = (member: Member) => {
    router.put(`/members/${member.id}`, { ...member }, { preserveScroll: true, onSuccess: () => addToast('Member updated') });
  };
  const onDeleteMember = (id: string) => {
    router.delete(`/members/${id}`, { preserveScroll: true, onSuccess: () => addToast('Member deleted', 'info') });
  };
  ...
  const allPossibleGroups = useMemo(() => {
    const defaultGroups = ['Youth Fellowship', 'Women of Grace', 'Men of Valor', 'Worship Team', 'Media & Tech', "Children's Ministry"];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-slate-800 tracking-tight text-brand-primary uppercase">Members</h2>
          </div>
          <p className="text-slate-500 mt-1 text-sm sm:text-lg font-medium">Manage church member records and details.</p>
        </div>
      ...
      <div className="bg-white rounded-[2rem] sm:rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden min-h-[400px]">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="text-slate-400 text-[10px] uppercase font-black tracking-widest border-b border-slate-100">
                <th className="px-6 sm:px-10 py-5 w-[35%]">Member</th>
                <th className="px-6 sm:px-10 py-5 w-[25%] hidden sm:table-cell">Contact</th>
                <th className="px-6 sm:px-10 py-5 w-[20%] hidden md:table-cell">Groups</th>
                <th className="px-6 sm:px-10 py-5 w-[12%] hidden sm:table-cell">Status</th>
                <th className="px-6 sm:px-10 py-5 w-[8%] text-right">Actions</th>
              </tr>
            </thead>
                ...
                <tr key={member.id} className="hover:bg-slate-50/80 transition-all">
                  <td className="px-6 sm:px-10 py-4">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-[0.75rem] border-2 border-brand-primary/5 overflow-hidden bg-white flex-shrink-0">
                        {member.photo ? <img src={member.photo} alt="" className="w-full h-full object-cover" /> : <User className="w-full h-full p-2.5 text-slate-200" />}
                      </div>
                      <div className="min-w-0">
                        <p className="font-black text-slate-800 text-sm leading-tight truncate">{member.firstName} {member.lastName}</p>
                        <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest truncate">{member.membershipType}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 sm:px-10 py-4 hidden sm:table-cell">
                    <p className="text-sm text-slate-700 font-bold truncate">{member.phone}</p>
                    <p className="text-[10px] text-slate-400 font-medium truncate">{member.location}</p>
                  </td>
                  <td className="px-6 sm:px-10 py-4 hidden md:table-cell">
                    <div className="flex flex-wrap gap-1.5">
                      {member.groups?.slice(0, 2).map(g => (
                        <span key={g} className="px-2 py-0.5 bg-brand-gold/5 border border-brand-gold/10 text-slate-500 text-[8px] font-black uppercase rounded-lg whitespace-nowrap">{g}</span>
                      ))}
                      {(member.groups?.length || 0) > 2 && (
                        <span className="text-[8px] font-black text-slate-300">+{member.groups!.length - 2}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-6 sm:px-10 py-4 hidden sm:table-cell">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${
                  ...
                  <td className="px-6 sm:px-10 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button 
                        ...
                        className="p-2 bg-brand-indigo/5 text-brand-indigo rounded-lg hover:bg-brand-indigo hover:text-white transition-all"
                        title="View Profile"
                      >
                        <Eye size={15} />
                      </button>
                      {currentUserRole !== UserRole.MEMBER && (
                        <div className="relative">
                          <button 
                            ...
                            className={`p-2 rounded-lg transition-all ${openActionMenuId === member.id ? 'bg-slate-200 text-slate-900' : 'bg-slate-50 text-slate-300 hover:text-slate-600'}`}
                          >
                            <MoreHorizontal size={15} />
                          </button>
                          {openActionMenuId === member.id && (
                            <div className="absolute right-0 top-full mt-1.5 w-44 bg-white border border-slate-100 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                              <button 
                                onClick={() => { setShowEditModal(member); setFormData(member); setPhoto(member.photo || null); setOpenActionMenuId(null); }}
                                className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-700 hover:bg-brand-indigo/5 flex items-center gap-2.5 transition-colors border-b border-slate-50"
                              >
                                <Edit2 size={13} className="text-brand-indigo"/> Edit
                              </button>
                              <button 
                                onClick={() => { setMessagingGroup({id: member.id, name: `${member.firstName} ${member.lastName}`} as any); setOpenActionMenuId(null); }}
                                className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-700 hover:bg-brand-emerald/5 flex items-center gap-2.5 transition-colors border-b border-slate-50"
                              >
                                <MessageCircle size={13} className="text-brand-emerald"/> Send SMS
                              </button>
                              <button 
                                onClick={() => { setShowDeleteConfirm(member); setOpenActionMenuId(null); }}
                                className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest text-brand-gold hover:bg-brand-gold/5 flex items-center gap-2.5 transition-colors"
                              >
                                <Trash2 size={13} className="text-brand-gold"/> Delete
                              </button>
                            </div>
                          )}
                        </div>
        ...
        {filteredMembers.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="font-black text-slate-300 text-lg uppercase tracking-widest">No members found</p>
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl p-10 space-y-8 animate-in zoom-in-95 duration-300">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-3xl font-black text-slate-800 uppercase tracking-tight leading-none">Bulk Import</h3>
            ...
            <form onSubmit={showAddModal ? handleAddSubmit : handleEditSubmit} className="flex flex-col h-full lg:max-h-[90vh]">
              <div className="p-8 sm:p-12 bg-slate-50 border-b border-slate-100 flex justify-between items-center relative overflow-hidden flex-shrink-0">
                <div className="flex items-center gap-6 relative z-10">
                  ...
                  <div>
                    <h3 className="text-2xl sm:text-4xl font-black text-slate-800 uppercase tracking-tight">{showAddModal ? 'Add New Member' : 'Update Profile'}</h3>
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-1">Member Registration</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                   <div className="lg:w-1/3 space-y-8">
                      <div className="space-y-4">
                        <label className="text-[11px] font-black uppercase text-slate-400 tracking-widest ml-2">Photo</label>
                      ...
                      <div className="space-y-4 p-6 bg-slate-100/50 rounded-[2rem] border border-slate-200">
                        <label className="text-[11px] font-black uppercase text-slate-400 tracking-widest ml-2">Groups</label>
                   ...
                   <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-2">
                        <label className="text-[11px] font-black uppercase text-slate-400 tracking-widest ml-2">Membership Type</label>
                      ...
                      <div className="space-y-2">
                        <label className="text-[11px] font-black uppercase text-slate-400 tracking-widest ml-2">Status</label>
              ...
              <div className="p-10 sm:p-14 border-t border-slate-100 bg-white flex flex-col sm:flex-row gap-6 flex-shrink-0">
                <button type="button" onClick={() => { setShowAddModal(false); setShowEditModal(null); setFormData({ groups: [] }); }} className="flex-1 py-6 font-black text-slate-400 hover:bg-slate-50 rounded-[1.5rem] uppercase text-xs tracking-widest transition-all">Cancel</button>
                <button 
                  type="submit" 
                  disabled={isSaving}
                ...
                >
                  {isSaving ? <Loader2 className="animate-spin" size={24}/> : <CheckCircle2 size={24}/>} {showAddModal ? 'Save' : 'Update Record'}
                </button>
              </div>
            </form>
          </div>
      ...
      {showDeleteConfirm && (
         <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[700] flex items-center justify-center p-4">
            <div className="bg-white rounded-[3rem] w-full max-md shadow-2xl p-12 space-y-10 animate-in zoom-in-95 duration-200 text-center border border-white/20">
               ...
               <div className="space-y-4">
                  <h3 className="text-3xl font-black text-slate-800 uppercase tracking-tight leading-tight">Delete Member?</h3>
                  <p className="text-slate-500 font-medium text-base leading-relaxed px-4">You are about to delete the member record for <span className="text-slate-900 font-black">{showDeleteConfirm.firstName} {showDeleteConfirm.lastName}</span>. This action is irreversible.</p>
               </div>
               <div className="flex flex-col gap-4">
                  ...
                  <button onClick={() => setShowDeleteConfirm(null)} className="w-full py-5 bg-slate-50 text-slate-400 rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-slate-100 transition-all">Cancel</button>
               </div>
            </div>
         </div>
      )}
