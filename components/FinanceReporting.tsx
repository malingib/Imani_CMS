  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-4xl font-black text-slate-800 tracking-tight text-brand-primary uppercase">Finance Management</h2>
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button onClick={() => setShowMPesaModal(true)} className="flex items-center gap-2 px-6 py-2.5 bg-brand-primary text-white rounded-2xl font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          ...
          <h4 className="text-4xl font-black text-brand-secondary mt-4 tracking-tighter">KES {totalExpenses.toLocaleString()}</h4>
          <p className="text-xs text-slate-400 mt-2">Target savings: 30%</p>
        </div>
        <div className="bg-brand-solid p-8 rounded-[2.5rem] shadow-xl text-white">
          <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Church Net Fund</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center">
            <div className="flex-1 min-w-[300px] relative">
              ...
              <input 
                ...
                className="w-full pl-12 pr-6 py-3 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 outline-none focus:ring-2 focus:ring-brand-primary"
                value={searchTerm}
                   ...
                   <tbody className="divide-y divide-slate-50">
                      {filtered.map(t => (
                        <tr key={t.id} className="hover:bg-slate-50 transition-all group">
                           <td className="px-8 py-6 font-mono text-xs text-brand-primary font-bold">{t.reference}</td>
                           ...
                           <td className="px-8 py-6">
                              <div className="flex gap-2">
                                 <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${t.category === 'Expense' ? 'bg-sky-100 text-sky-700' : 'bg-emerald-100 text-emerald-700'}`}>{t.type}</span>
                           ...
                           <td className={`px-8 py-6 text-right font-black text-lg ${t.category === 'Expense' ? 'text-brand-secondary' : 'text-slate-900'}`}>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
             <h4 className="font-black text-slate-800 mb-6 flex items-center gap-3"><BarChart3 size={20} className="text-brand-primary"/> Budget vs Actual</h4>
             <div className="h-64">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
             <h4 className="font-black text-slate-800 mb-6 flex items-center gap-3"><PieIcon size={20} className="text-brand-gold"/> Income Mix</h4>
             <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                   <PieChart>
          ...
          <div className="bg-brand-solid p-8 rounded-[2.5rem] text-white">
             <h4 className="font-black mb-6 flex items-center gap-3 text-slate-300"><Repeat size={20}/> Active Recurring Dues</h4>
                ...
                {recurringExpenses.map(r => (
                  <div key={r.id} className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/10">
                     <div>
                        ...
                        <p className="text-[10px] text-slate-400 uppercase tracking-widest">{r.frequency} • {r.category}</p>
                     </div>
                     <p className="font-black text-brand-gold">KES {r.amount.toLocaleString()}</p>
                  </div>
                ))}
