    ...
    <div className="max-w-4xl mx-auto py-12 px-6 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
           <div className="p-4 bg-brand-indigo/10 text-brand-indigo rounded-2xl w-fit"><Key size={32}/></div>
           ...
           <div className="space-y-3 pt-4 border-t border-slate-50">
              {['End-to-End Database Encryption', 'Bcrypt Hashed Passwords', 'Automated Daily Backups'].map(item => (
                <div key={item} className="flex items-center gap-3 text-xs font-bold text-slate-600">
                   <CheckCircle2 size={16} className="text-brand-emerald" /> {item}
                </div>
              ))}
           </div>
        </div>
        ...
        <div className="bg-brand-primary p-10 rounded-[3rem] text-white shadow-xl space-y-6 relative overflow-hidden">
           ...
           <div className="absolute top-[-20%] right-[-20%] w-64 h-64 bg-brand-indigo rounded-full blur-[80px] opacity-20"></div>
        </div>
      </div>
