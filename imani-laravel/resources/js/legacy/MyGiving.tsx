...
const MyGiving: React.FC<MyGivingProps> = ({ member, transactions, onGive }) => {
  ...
  const handleMpesaTrigger = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      const { checkoutRequestId, message } = await mpesa.stkPushGiving(amount, giftType, member.phone);
      ...
      if (result.status === 'completed') {
        ...
        setPaymentMessage('Payment was not completed. Check your phone and try again.');
      }
    } catch (err) {
      setPaymentMessage(err instanceof Error ? err.message : 'M-Pesa payment failed.');
    } finally {
