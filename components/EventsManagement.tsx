          ...
          <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl p-10 space-y-8 animate-in zoom-in duration-200 flex flex-col max-h-[85vh]">
            ...
            <div className="relative">
               ...
               <input 
                ...
                className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold"
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-2 pr-2">
               {members.filter(m => `${m.firstName} ${m.lastName}`.toLowerCase().includes(attendanceSearch.toLowerCase())).map(m => {
                 ...
                 return (
                   ...
                   >
                     <div className="flex items-center gap-4">
                       <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs ${isPresent ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                          {m.firstName[0]}{m.lastName[0]}
                       </div>
                       <div className="text-left">
                          <p className={`text-sm font-bold ${isPresent ? 'text-indigo-900' : 'text-slate-700'}`}>{m.firstName} {m.lastName}</p>
                          <p className="text-[10px] text-slate-400 uppercase font-black">{m.group}</p>
                       </div>
                     </div>
                     <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all ${isPresent ? 'bg-indigo-600 text-white rotate-0' : 'border-2 border-slate-100 rotate-45'}`}>
            ...
            <div className="pt-6 border-t border-slate-50 flex justify-between items-center">
               ...
               <div className="flex gap-3">
                 ...
                 <button onClick={saveRollCall} className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-black shadow-lg hover:bg-indigo-700">Save Attendance</button>
               </div>
            </div>
          </div>
        </div>
