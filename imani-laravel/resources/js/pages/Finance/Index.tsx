...
export default function FinanceIndex({
  ...
  const onAddTransaction = (trx: Transaction) => {
    router.post('/finance/transactions', { ...trx }, { preserveScroll: true, onSuccess: () => addToast('Transaction saved') });
  };
  const onUpdateTransaction = (trx: Transaction) => {
    router.put(`/finance/transactions/${trx.id}`, { ...trx }, { preserveScroll: true, onSuccess: () => addToast('Transaction updated') });
  };
  const onDeleteTransaction = (id: string) => {
    router.delete(`/finance/transactions/${id}`, { preserveScroll: true, onSuccess: () => addToast('Transaction deleted', 'info') });
  };
  const onSetBudget = (_budget: Budget) => {};
  const onAddRecurring = (_expense: RecurringExpense) => {};
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm min-h-[500px]">
             <div className="p-8 border-b border-slate-100 bg-slate-50/30 flex justify-between items-center">
                <h3 className="text-xl font-black text-brand-primary uppercase tracking-tight">Transactions</h3>
                <div className="flex items-center gap-2">
                   <div className="w-2 h-2 rounded-full bg-brand-emerald animate-pulse" />
                    <span className="text-[10px] font-black uppercase text-brand-emerald tracking-widest">Active</span>
                </div>
             </div>
             ...
             {filtered.length > 0 ? (
                                  ...
                                  <div className="absolute right-10 top-full mt-2 w-48 bg-white border border-slate-100 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                                     ...
                                     {t.source === 'INTEGRATED' && (
                                       <div className="px-6 py-3 bg-slate-50">
                                           <p className="text-[8px] font-black uppercase text-slate-400 leading-tight">Integrated (read-only)</p>
                                       </div>
                                     )}
                                  </div>
                                )}
               ...
               <div className="py-32 text-center space-y-6">
                  ...
                  <div>
                    <h4 className="text-xl font-black text-slate-400 uppercase">No Transactions</h4>
        ...
        <div className="lg:col-span-4 space-y-8">
           ...
           <div className="bg-slate-50 border-2 border-dashed border-slate-200 p-8 rounded-[2.5rem] flex flex-col items-center justify-center text-center space-y-4">
              <Landmark size={48} className="text-slate-200" />
               <h4 className="text-lg font-black text-slate-500 uppercase">Tax Compliance</h4>
              ...
              <div className="p-8 sm:p-12 space-y-10 relative">
                 ...
                 <div className="flex flex-col items-center text-center space-y-4">
                    ...
                    <div>
                       <h3 className="text-3xl font-black text-slate-800 uppercase tracking-tight">Transaction Detail</h3>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Transaction Details</p>
                    </div>
                 </div>
                 ...
                 <div className="bg-slate-50 rounded-[2.5rem] p-8 space-y-6 border border-slate-100">
                    ...
                    <div className="pt-4 flex justify-between items-end">
                       <span className="text-lg font-black uppercase text-brand-primary tracking-tight">Amount</span>
                       <div className="text-right">
             ...
             <form onSubmit={handleFormSubmit} className="flex flex-col h-full max-h-[95vh]">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h3 className="text-2xl sm:text-3xl font-black text-slate-800 uppercase tracking-tight">
                       ...
                       <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-1">Manual Entry</p>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 sm:p-12 lg:p-16 space-y-10 no-scrollbar">
                   ...
                   <div className="p-6 bg-brand-gold/5 rounded-[2rem] border border-dashed border-brand-gold/20 flex items-start gap-4">
                      <ShieldCheck className="text-brand-gold shrink-0 mt-1" size={20}/>
                       <p className="text-[11px] text-slate-600 font-medium leading-relaxed">Manual entries bypass integrated payment logic. Ensure physical receipts match the ledger reference to maintain accurate records.</p>
                   </div>
                </div>
                ...
                <div className="p-10 sm:p-14 border-t border-slate-100 bg-white flex flex-col sm:flex-row gap-4 flex-shrink-0">
                  ...
                  >
                    ...
                    {isProcessing ? 'Syncing...' : (typeof showFormModal === 'string' ? 'Save Transaction' : 'Update Transaction')}
                  </button>
                </div>
             </form>
          </div>
